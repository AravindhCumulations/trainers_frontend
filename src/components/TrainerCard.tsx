        ...
        >
            ...
            <div className="rounded-2xl overflow-hidden mb-3 flex items-center justify-center bg-gray-100 relative h-[192px] w-[260px] trainer-card-avatar">
                ...
                <button
                    ...
                    className="absolute top-2 right-2 bg-white/80 rounded-full p-1 shadow hover:bg-red-100 transition trainer-card-fav-btn"
            ...
            <div className="text-left w-full flex-grow">
                <span className="block text-[18px] flex gap-2 font-bold text-[#111827] leading-[28px] mb-0.5 trainer-card-name">
                    {trainer.full_name}
                ...
                <span className="block text-[#EAB308]" aria-label={`${trainer.avg_rating} out of 5 stars`}>
                    {'★'.repeat(Math.round(trainer.avg_rating))}{'☆'.repeat(5 - Math.round(trainer.avg_rating))}
                </span>

                <div className="text-[#4B5563]">
                    <span className="block text-[14px] font-normal leading-[20px] mb-1 flex items-center gap-1 trainer-card-skills line-clamp-2">
                    ...
                    <span className="block text-[14px] font-normal leading-[20px] mb-1 trainer-card-exp line-clamp-1">
                        {trainer.experience} years   • {trainer.city}
                    </span>

                    <span className="block text-[14px] font-normal leading-[20px] mb-1 trainer-card-lang line-clamp-1">
                        {trainer.language}
                    </span>
                </div>
            </div>
            ...
            {/* Trainer bottom info */}
            <div className="flex items-center w-full justify-between mt-auto trainer-card-bottom h-[30px]">
                <span className="text-[14px] font-semibold text-[#111827] leading-[20px] trainer-card-price">
                    ₹{trainer.charge}/hour
                </span>
                <span className="flex items-center text-[14px] font-normal text-[#6B7280] leading-[20px] trainer-card-views justify-between gap-[4px] text-center">
                    <svg width="17" height="16" viewBox="0 0 17 16" fill="none" xmlns="http://www.w3.org/2000/svg" className="flex items-center">
